...
(function() {
    ...
    content = content.replace(/((?:srcset|data-srcset|dt)\s*=\s*["'])([^"']+)(["'])/gi, (match, prefix, urls, suffix) => {
        ...
        const rewrittenUrls = urls.replace(/https?:\/\/([^\/\s,]+)([^\s,]*)/g, (urlMatch, domain, path) => {
            if (domain === proxyHost || path.includes('hmtarget=')) return urlMatch;
            const separator = path.includes('?') ? '&' : '?';
    ...
    content = content.replace(/url\s*\(\s*["']?([^"')]+)["']?\s*\)/gi, (match, url) => {
        if (url.startsWith('data:') || url.startsWith('#') || url.includes('hmtarget=')) return match;
    ...
    content = content.replace(/(['"`])\/\/([^\/\s'"`]+)([^'"`]*)\1/g, (match, quote, domain, path) => {
        if (match.includes('/*') || match.includes('//') || domain === proxyHost || path.includes('hmtarget=')) {
            return match;
        }
...
app.use('/cart', (req, res, next) => {
    ...
    if (cartRequestTracker.has(key)) {
        ...
        if (now - lastRequest < 500) { // 500ms cooldown for cart requests
...
app.listen(PORT, () => {
    ...
    console.log(`Asset proxy: http://localhost:${PORT}/asset?hmtarget=example.com&hmtype=2`);
    console.log(`=================================`);
});
